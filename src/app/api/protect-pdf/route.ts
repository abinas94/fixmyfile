...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Step 2: Convert PDF to PDF with password protection
    ...
    if (!convertRes.ok) {
      // Fallback: try with libreoffice engine
      const fallbackRes = await fetch(`${BASE_URL}/convert`, {
        method: "POST",
        headers: { Authorization: `Bearer ${API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({
          input: [uploadTaskId],
      ...
      if (!fallbackRes.ok) return NextResponse.json({ error: "Protection failed" }, { status: 502 });
      const fallbackData = await fallbackRes.json();

      // Wait + export (same flow)
      let taskId = fallbackData.data.id;
      let attempts = 0;
      while (attempts < 30) {
        await new Promise((r) => setTimeout(r, 2000)); attempts++;
        const s = await fetch(`${BASE_URL}/tasks/${taskId}`, { headers: { Authorization: `Bearer ${API_KEY}` } });
        const d = await s.json();
        if (d.data.status === "finished") break;
        if (d.data.status === "error") return NextResponse.json({ error: "Failed" }, { status: 502 });
      }

      const expRes = await fetch(`${BASE_URL}/export/url`, {
        method: "POST",
        headers: { Authorization: `Bearer ${API_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ input: [taskId] }),
      });
      const expData = await expRes.json();
      let expId = expData.data.id;
      let exportUrl = "";
      attempts = 0;
      while (attempts < 15) {
        await new Promise((r) => setTimeout(r, 1500)); attempts++;
        const s = await fetch(`${BASE_URL}/tasks/${expId}`, { headers: { Authorization: `Bearer ${API_KEY}` } });
        const d = await s.json();
      ...
      fetch(`${BASE_URL}/tasks/${taskId}`, { method: "DELETE", headers: { Authorization: `Bearer ${API_KEY}` } }).catch(() => {});
      return new NextResponse(buf, { headers: { "Content-Type": "application/pdf", "Content-Disposition": 'attachment; filename="protected.pdf"' } });
    }
    ...
    while (attempts < 30) {
      ...
      const d = await s.json();
      if (d.data.status === "finished") break;
      if (d.data.status === "error") return NextResponse.json({ error: d.data.message || "Failed" }, { status: 502 });
    }
