...
export default function PPTToPDF() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const blob = await convertFile(files[0], "pptx", "pdf", setProgress);
      downloadBlob(blob, files[0].name.replace(/\.pptx?$/i, ".pdf"));
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center shadow-lg"><FileText className="w-6 h-6 text-white" /></div>
          <div><h1 className="text-2xl sm:text-3xl font-bold">PPT to PDF</h1><p className="text-[var(--muted-foreground)]">Convert PowerPoint to PDF with all slides and formatting</p></div>
        </div>
      </div>
      ...
      {files.length > 0 && (
        <div className="mt-8 flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-[var(--muted-foreground)]">All slides, images, animations, and formatting preserved in PDF.</p>
        </div>
      )}
    </div>
  );
