...
export default function PDFToWord() {
  ...
  const handleConvert = async () => {
    ...
    setProgress("Converting PDF to editable Word...");
    try {
      ...
      alert("Error: " + (error instanceof Error ? error.message : "Conversion failed. Please try again."));
      setProgress("");
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--muted-foreground)]">Convert PDF to fully editable Word document</p>
          </div>
        </div>
      </div>
      ...
      {files.length > 0 && (
        <div className="mt-6 flex flex-col items-center gap-4">
          ...
          <p className="text-xs text-[var(--muted-foreground)] text-center max-w-md">
            Produces a fully editable .docx with text, images, tables, and formatting preserved. Opens in Microsoft Word, Google Docs, or LibreOffice.
          </p>
        </div>
      )}
    </div>
